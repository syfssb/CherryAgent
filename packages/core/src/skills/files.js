...
export function buildFrontmatter(meta) {
    ...
    if (meta.skillId)
        lines.push(`skillId: ${meta.skillId}`);
    if (meta.managedBy)
        lines.push(`managedBy: ${meta.managedBy}`);
    if (meta.author)
...
function getManagedDirName(skillId) {
...
export class SkillFileManager {
    ...
    /** 写入数据库管理的 skill 文件 */
    writeManagedSkillFile(skill, content) {
    ...
    /** 删除数据库管理的 skill 文件 */
    removeManagedSkillFile(skillId) {
        const sdkSkillsDir = this.getSDKSkillsDir();
    ...
    listPresetSkills() {
        ...
        if (!resourceDir || !existsSync(resourceDir))
            return presets;
    ...
    scanUserCreatedSkills() {
        ...
        for (const entry of readdirSync(sdkSkillsDir, { withFileTypes: true })) {
            ...
            if (meta.managedBy === "preset")
                continue;
            if (meta.managedBy === "db")
                continue;
            if (meta.managedBy === "remote-preset")
                continue;
