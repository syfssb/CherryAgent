...
function stripErrorPrefix(message, prefix) {
...
export class CloudSyncService {
    ...
    loadConfig(apiBaseUrl) {
        ...
        if (existsSync(this.configPath)) {
            try {
                config = JSON.parse(readFileSync(this.configPath, "utf8"));
            }
            catch { /* ignore */ }
        }
        const allowedEntities = ["session", "tag", "memory_block", "skill", "setting"];
        const persistedEntities = Array.isArray(config.enabledEntities)
            ? config.enabledEntities.filter((entity) => allowedEntities.includes(entity))
            : [];
        const normalizedEntities = persistedEntities.length > 0
            ? Array.from(new Set([...persistedEntities, "setting"]))
    ...
    setProgressCallback(callback) { this.onProgressCallback = callback; }
    ...
    getPendingChangesCount() {
        const result = this.db.prepare("SELECT COUNT(*) as count FROM sync_changes WHERE synced = 0").get();
        return result.count;
    }
    getUnresolvedConflictsCount() {
        const result = this.db.prepare("SELECT COUNT(*) as count FROM sync_conflicts WHERE resolved_at IS NULL").get();
        return result.count;
    }
    recordChange(entityType, entityId, changeType, data) {
        if (!this.config.enabledEntities.includes(entityType))
            return;
        const id = crypto.randomUUID();
        const timestamp = Date.now();
        const checksum = calculateChecksum(data);
        this.db.prepare(`INSERT INTO sync_changes (id, entity_type, entity_id, change_type, timestamp, checksum, synced) VALUES (?, ?, ?, ?, ?, ?, 0)`).run(id, entityType, entityId, changeType, timestamp, checksum);
    }
    getPendingChanges() {
        const rows = this.db.prepare(`SELECT id, entity_type, entity_id, change_type, timestamp, checksum, synced FROM sync_changes WHERE synced = 0 ORDER BY timestamp ASC`).all();
        return rows.map((row) => ({
            id: String(row.id),
    ...
    compactPendingChanges(pendingChanges) {
    ...
    async pushChanges() {
        ...
        console.log(`[CloudSyncService] 压缩待同步变更: ${pendingChanges.length} -> ${compactedChanges.length}`);
        const relatedChangeIdsByLatestId = new Map();
        ...
        try {
            ...
            for (let index = 0; index < changes.length; index += PUSH_BATCH_SIZE) {
                ...
                const result = unwrapApiData(responsePayload ?? {});
                ...
                const syncedIds = Array.isArray(result.syncedIds)
                    ? result.syncedIds.filter((id) => typeof id === "string")
                    : [];
            ...
            const normalizedMessage = stripErrorPrefix(error instanceof Error ? error.message : String(error), "Push failed: ");
    ...
    async pullChanges() {
        ...
        try {
            const response = await fetch(`${this.config.apiBaseUrl}/sync/pull?since=${since}&deviceId=${this.config.deviceId}`, { method: "GET", headers: { Authorization: `Bearer ${this.accessToken}` } });
            if (!response.ok)
            ...
            for (const change of changes) {
                ...
                if (applyResult === "applied")
                    ...
                    conflicts++;
                else if (applyResult === "auto_resolved")
                    autoResolved++;
            }
    ...
    applyRemoteChange(change) {
        const localChange = this.db.prepare(`SELECT timestamp, checksum FROM sync_changes WHERE entity_type = ? AND entity_id = ? AND synced = 0 ORDER BY timestamp DESC LIMIT 1`).get(change.entityType, change.entityId);
        // 检测到冲突
    ...
    createConflict(remoteChange) {
        ...
        this.db.prepare(`INSERT INTO sync_conflicts (id, entity_type, entity_id, local_data, remote_data, local_timestamp, remote_timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, remoteChange.entityType, remoteChange.entityId, JSON.stringify(localData), JSON.stringify(remoteChange.data), Date.now(), remoteChange.timestamp);
    }
    getEntityData(entityType, entityId) {
        switch (entityType) {
            case "session": {
                const row = this.db
                    .prepare(`SELECT id, title, claude_session_id, status, cwd, allowed_tools, active_skill_ids, skill_mode, last_prompt, is_pinned, is_archived, created_at, updated_at
             FROM sessions
             WHERE id = ?`)
                    .get(entityId);
                if (!row) {
            ...
            case "memory_block": {
                const row = this.db
                    .prepare(`SELECT id, label, description, value, char_limit, created_at, updated_at
             FROM memory_blocks
             WHERE id = ?`)
                    .get(entityId);
                if (!row) {
            ...
            case "skill": {
                const row = this.db
                    .prepare(`SELECT id, name, description, content, source, is_enabled, icon, category, created_at, updated_at
             FROM skills
             WHERE id = ?`)
                    .get(entityId);
                if (!row) {
    ...
    applyEntityChange(entityType, entityId, changeType, data) {
    ...
    upsertSession(session) {
        ...
        this.db
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(sessionId, getStringValue(source, "title") ?? "", getStringValue(source, "claudeSessionId", "claude_session_id") ?? null, getStringValue(source, "status") ?? "idle", getStringValue(source, "cwd") ?? null, getStringValue(source, "allowedTools", "allowed_tools") ?? null, activeSkillIdsValue, getStringValue(source, "skillMode", "skill_mode") ?? "auto", getStringValue(source, "lastPrompt", "last_prompt") ?? null, (getBooleanValue(source, "isPinned", "is_pinned") ?? false) ? 1 : 0, (getBooleanValue(source, "isArchived", "is_archived") ?? false) ? 1 : 0, createdAt, updatedAt);
        if (Array.isArray(tagIds)) {
            this.db.prepare("DELETE FROM session_tags WHERE session_id = ?").run(sessionId);
            const insertSessionTag = this.db.prepare(`INSERT OR IGNORE INTO session_tags (session_id, tag_id, created_at)
         SELECT ?, id, ?
         FROM tags
         WHERE id = ?`);
    ...
    upsertTag(tag) {
        ...
        this.db
            ...
            .run(tagId, getStringValue(source, "name") ?? "", getStringValue(source, "color") ?? "#999999", getNumberValue(source, "createdAt", "created_at") ?? Date.now());
    }
    upsertMemoryBlock(block) {
        ...
        const createdAt = getNumberValue(source, "createdAt", "created_at") ?? Date.now();
        this.db
            .prepare(`INSERT OR REPLACE INTO memory_blocks
         (id, label, description, value, char_limit, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(blockId, getStringValue(source, "label") ?? "", getStringValue(source, "description") ?? "", getStringValue(source, "value") ?? "", getNumberValue(source, "charLimit", "char_limit") ?? 2000, createdAt, getNumberValue(source, "updatedAt", "updated_at") ?? createdAt);
    }
    upsertSkill(skill) {
        ...
        const createdAt = getNumberValue(source, "createdAt", "created_at") ?? Date.now();
        this.db
            .prepare(`INSERT OR REPLACE INTO skills
         (id, name, description, content, source, is_enabled, icon, category, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(skillId, getStringValue(source, "name") ?? "", getStringValue(source, "description") ?? "", getStringValue(source, "content") ?? "", getStringValue(source, "source") ?? "custom", (getBooleanValue(source, "isEnabled", "is_enabled") ?? true) ? 1 : 0, getStringValue(source, "icon") ?? null, getStringValue(source, "category") ?? "other", createdAt, getNumberValue(source, "updatedAt", "updated_at") ?? createdAt);
    }
    upsertSetting(setting) {
        ...
        this.db
            ...
            .run(key, getStringValue(source, "value") ?? "", getNumberValue(source, "updatedAt", "updated_at") ?? Date.now());
    }
    ...
    getUnresolvedConflicts() {
        const rows = this.db.prepare(`SELECT id, entity_type, entity_id, local_data, remote_data, local_timestamp, remote_timestamp FROM sync_conflicts WHERE resolved_at IS NULL ORDER BY local_timestamp DESC`).all();
        return rows.map((row) => ({
            id: String(row.id),
            entityType: String(row.entity_type),
            entityId: String(row.entity_id),
    ...
    resolveConflict(conflictId, resolution) {
        const conflict = this.db.prepare(`SELECT id, entity_type, entity_id, local_data, remote_data FROM sync_conflicts WHERE id = ?`).get(conflictId);
        if (!conflict)
            return false;
