...
interface MutableTaskEntry<T = unknown> {
  id: string;
  type: TaskType;
...
export class TaskManager {
  ...
  private readonly tasks = new Map<string, MutableTaskEntry>();
  private readonly listeners = new Map<TaskEventType, Set<TaskEventListener<TaskEventType>>>();
  ...
  constructor(config?: TaskManagerConfig) {
    ...
    this.queue.on('idle', () => {
      this.emit('queue.idle', {} as Record<string, never>);
    });
  }
  ...
  submit<T>(
    ...
    // 创建任务条目
    const entry: MutableTaskEntry<T> = {
      id,
      type,
      priority,
      status: 'pending',
    ...
    // 将任务加入 p-queue
    const resultPromise = this.queue.add(
      async ({ signal: pqSignal }) => {
        ...
        if (abortController.signal.aborted) {
          throw new Error(
            abortController.signal.reason ?? 'Task was cancelled before execution',
          );
        }
   ...
   * 取消所有待执行和正在执行的任务
   */
  cancelAll(reason = 'All tasks cancelled'): number {
    this.assertNotDisposed();
   ...
   * 注册事件监听器
   */
  on<T extends TaskEventType>(type: T, listener: TaskEventListener<T>): () => void {
    if (!this.listeners.has(type)) {
      this.listeners.set(type, new Set());
    }
    const set = this.listeners.get(type)!;
    set.add(listener as TaskEventListener<TaskEventType>);

    // 返回取消订阅函数
    return () => {
      set.delete(listener as TaskEventListener<TaskEventType>);
    };
  }
   ...
   * 移除事件监听器
   */
  off<T extends TaskEventType>(type: T, listener: TaskEventListener<T>): void {
    const set = this.listeners.get(type);
    if (set) {
      set.delete(listener as TaskEventListener<TaskEventType>);
    }
   ...
   */
  private cancelInternal(id: string, reason: string): boolean {
    const entry = this.tasks.get(id);
    if (!entry) {
      return false;
    }
   ...
   */
  private cancelAllInternal(reason: string): number {
    let count = 0;
  ...
  private emit<T extends TaskEventType>(
    type: T,
    ...
    const event = {
      ...
      payload,
      timestamp: Date.now(),
    } as unknown as Parameters<TaskEventListener<TaskEventType>>[0];
  ...
  private toTaskInfo<T>(entry: MutableTaskEntry<T>): TaskInfo<T> {
    return {
      id: entry.id,
      type: entry.type,
  ...
  private pruneCompletedTasks(): void {
    ...
    const terminalEntries: Array<{ id: string; completedAt: number }> = [];
