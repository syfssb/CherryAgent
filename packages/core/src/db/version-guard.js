...
export class VersionGuard {
    ...
    check() {
        ...
        if (dbVersion > this.appMaxVersion) {
            return {
                ...
                message: `数据库版本 (v${dbVersion}) 高于当前应用支持的最高版本 (v${this.appMaxVersion})。` +
        ...
        if (dbVersion < this.appMinVersion) {
            return {
                ...
                message: `数据库版本 (v${dbVersion}) 低于当前应用支持的最低版本 (v${this.appMinVersion})。` +
        ...
        return {
            ...
            appMinVersion: this.appMinVersion,
            action: 'migrate-up',
            message: `数据库版本 (v${dbVersion}) 需要迁移到 v${this.appMaxVersion}。` +
                `将在迁移前自动创建备份。`
        };
    }
    /**
    ...
    createBackup(backupDir) {
        ...
        try {
            ...
            console.warn('[version-guard] VACUUM INTO 失败，尝试 backup API:', error instanceof Error ? error.message : String(error));
            try {
                this.db.backup(backupPath);
                console.info(`[version-guard] 备份已创建 (backup API): ${backupPath}`);
            }
            catch (backupError) {
                throw new Error(`数据库备份失败: ${backupError instanceof Error ? backupError.message : String(backupError)}`);
            }
        }
        // 清理旧备份
        ...
        try {
            ...
            for (const file of toDelete) {
                try {
                    ...
                    console.warn(`[version-guard] 删除旧备份失败: ${file.name}`, err instanceof Error ? err.message : String(err));
                }
            }
            if (toDelete.length > 0) {
                console.info(`[version-guard] 清理了 ${toDelete.length} 个旧备份，保留最近 ${this.maxBackups} 个`);
            }
        }
        catch (error) {
            // 清理失败不应阻止正常流程
            console.warn('[version-guard] 清理旧备份时出错:', error instanceof Error ? error.message : String(error));
        }
    }
    /**
    ...
    syncFromSchemaMigrations() {
        try {
            ...
            if (userVersion === 0 && schemaVersion > 0) {
                ...
                console.info(`[version-guard] 已从 schema_migrations 同步 user_version: 0 -> ${schemaVersion}`);
            }
        }
        catch (error) {
            // 同步失败不应阻止启动
            console.warn('[version-guard] 同步 schema_migrations 时出错:', error instanceof Error ? error.message : String(error));
        }
    }
}
