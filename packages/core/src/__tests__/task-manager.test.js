...
function createSlowTask(ms, result = 'done') {
    return async (signal) => {
...
describe('TaskManager', () => {
    ...
    describe('submit', () => {
        it('should execute a task and return result', async () => {
            const result = await manager.submit('task-1', 'session.start', async () => 'hello');
        ...
        it('should handle task failure', async () => {
            await expect(manager.submit('fail', 'test', async () => {
        ...
        it('should respect concurrency limit', async () => {
            let running = 0;
            let maxRunning = 0;
            const makeTask = (id) => manager.submit(id, 'test', async () => {
                running++;
    ...
    describe('timeout', () => {
        it('should cancel task after timeout', async () => {
            await expect(manager.submit('timeout-task', 'test', createSlowTask(5000), {
                timeoutMs: 50,
            })).rejects.toThrow();
    ...
    describe('external signal', () => {
        it('should respect external AbortSignal', async () => {
            ...
            const promise = manager.submit('ext-signal', 'test', createSlowTask(5000), { signal: external.signal });
            await delay(20);
        ...
        it('should handle pre-aborted signal', () => {
            ...
            const promise = manager.submit('pre-aborted', 'test', createSlowTask(100), { signal: external.signal });
