...
function createMockDb(options) {
    const { userVersion = 0, schemaMigrationsExists = false, schemaMaxVersion = null, } = options ?? {};
    let currentUserVersion = userVersion;
    const mockDb = {
        pragma: vi.fn((pragmaStr, opts) => {
        ...
        prepare: vi.fn((sql) => {
            ...
            if (sql.includes('sqlite_master') && sql.includes('schema_migrations')) {
                return {
                    get: vi.fn(() => schemaMigrationsExists ? { name: 'schema_migrations' } : undefined),
                };
            }
            // MAX(version) 查询
