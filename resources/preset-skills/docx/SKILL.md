...
When creating a new Word document from scratch, use **docx-js**, which allows you to create Word documents using JavaScript/TypeScript. The `docx` library is **built-in** and available via `$CHERRY_NODE` — no installation needed.

### Workflow
1. **MANDATORY - READ ENTIRE FILE**: Read [`docx-js.md`](docx-js.md) (~500 lines) completely from start to finish. **NEVER set any range limits when reading this file.** Read the full file content for detailed syntax, critical formatting rules, and best practices before proceeding with document creation.
2. Create a `.cjs` file using Document, Paragraph, TextRun components (the `docx` library is pre-installed via built-in runtime)
3. Export as .docx using Packer.toBuffer()
4. Run with: `ELECTRON_RUN_AS_NODE=1 "$CHERRY_NODE" create-doc.cjs`
...
**Built-in (no installation needed)** — available via `$CHERRY_NODE`:
- **docx** — Word document creation (`require('docx')` in `.cjs` files)

**System tools** (may need manual installation if not present):
- **pandoc** — text extraction and tracked changes (`pandoc --track-changes=all`)
