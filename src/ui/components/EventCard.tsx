...
import { MessageCost, type MessageUsageInfo } from "./chat/MessageCost";
...
const SessionResult = ({ message }: { message: SDKResultMessage }) => {
  ...
  const formatCredits = (usd: number | undefined) => typeof usd !== "number" ? "-" : `${usd.toFixed(2)} ${t("payment.creditsCost", "Credits")}`;
  const formatMillions = (tokens: number | undefined) => typeof tokens !== "number" ? "-" : `${(tokens / 1_000_000).toFixed(4)} M`;

  return (
    <div className="flex flex-col gap-2 mt-4">
      <div className="header text-accent">{t("chat.sessionResult", "Session result")}</div>
      <div className="flex flex-col rounded-xl px-4 py-3 border border-ink-900/10 bg-surface-secondary space-y-2">
        <div className="flex flex-wrap items-center gap-2 text-[14px]">
          <span className="font-normal">{t("chat.sessionDuration", "Duration")}</span>
          ...
          <span className="font-normal">{t("chat.apiDuration", "API")}</span>
        ...
        <div className="flex flex-wrap items-center gap-2 text-[14px]">
          <span className="font-normal">{t("chat.usage", "Usage")}</span>
          <span className="inline-flex items-center rounded-full bg-accent/10 px-2.5 py-0.5 text-accent text-[13px]">{t("chat.costWithValue", "Cost {{value}}", { value: formatCredits(message.total_cost_usd) })}</span>
          <span className="inline-flex items-center rounded-full bg-surface-tertiary px-2.5 py-0.5 text-ink-700 text-[13px]">{t("chat.inputWithValue", "Input {{value}}", { value: formatMillions(message.usage?.input_tokens) })}</span>
          <span className="inline-flex items-center rounded-full bg-surface-tertiary px-2.5 py-0.5 text-ink-700 text-[13px]">{t("chat.outputWithValue", "Output {{value}}", { value: formatMillions(message.usage?.output_tokens) })}</span>
        </div>
      </div>
    </div>
  );
...
const ToolResult = ({ messageContent }: { messageContent: ToolResultContent }) => {
  ...
  return (
    <div className="flex flex-col mt-4">
      <div className="header text-accent">{t("chat.outputLabel", "Output")}</div>
          ...
          <button onClick={() => setIsExpanded(!isExpanded)} className="mt-2 text-sm text-accent hover:text-accent-hover transition-colors flex items-center gap-1">
            ...
            <span>
              {isExpanded
                ...
                : t("chat.showMoreLines", "Show {{count}} more lines", { count: lines.length - MAX_VISIBLE_LINES })}
            </span>
          </button>
        )}
        <div ref={bottomRef} />
...
const ToolUseCard = ({ messageContent, showIndicator = false }: { messageContent: MessageContent; showIndicator?: boolean }) => {
  ...
  const hasFilePath = () => {
    return ["Read", "Write", "Edit"].includes(messageContent.name);
  };
  ...
  const copyFilePath = async () => {
    ...
    try {
      ...
      toast({ title: t("workspace.pathCopied", "路径已复制") });
    } catch (err) {
      toast({ title: t("workspace.copyFailed", "复制失败"), variant: "destructive" });
    }
  };
  ...
  const openFile = async () => {
    ...
    try {
      await window.electron?.openExternal?.(filePath);
    } catch (err) {
      // 静默处理错误
    }
  };
  ...
  return (
    <div className="group flex flex-col gap-2 rounded-[1rem] bg-surface-tertiary px-3 py-2 mt-4 overflow-hidden">
      <div className="flex flex-row items-center gap-2 min-w-0">
        ...
        {hasFilePath() && (
          <div className="hidden group-hover:flex gap-1 shrink-0">
            <button
              onClick={openFile}
              className="p-1 rounded hover:bg-ink-900/10 transition-colors"
              title={t("workspace.openFile", "打开文件")}
              aria-label={t("workspace.openFile", "打开文件")}
            >
...
const SystemInfoCard = ({ message, showIndicator = false }: { message: SDKMessage; showIndicator?: boolean }) => {
  ...
  return (
    <div className="flex flex-col gap-2 mt-2">
      <div className="header text-accent flex items-center gap-2">
        <StatusDot variant="success" isActive={showIndicator} isVisible={showIndicator} />
        {t("chat.systemInit", "System init")}
      </div>
      <div className="flex flex-col rounded-xl px-4 py-2 border border-ink-900/10 bg-surface-secondary space-y-1">
        <InfoItem name={t("chat.sessionId", "Session ID")} value={systemMsg.session_id || "-"} />
        <InfoItem name={t("chat.model", "Model")} value={systemMsg.model || "-"} />
        <InfoItem name={t("chat.permissionMode", "Permission mode")} value={systemMsg.permissionMode || "-"} />
        <InfoItem name={t("chat.workingDirectory", "Working directory")} value={systemMsg.cwd || "-"} />
      </div>
    </div>
  );
};
...
export function MessageCard({
  ...
  if (sdkMessage.type === "result") {
    ...
    return (
      <div className="flex flex-col gap-2 mt-4">
        <div className="header text-error">{t("chat.sessionError", "Session error")}</div>
    ...
    return (
      <>
        {contents.map((content: MessageContent, idx: number) => {
          ...
          if ((content as any).type === "thinking") {
            return <AssistantBlockCard key={idx} title={t("chat.thinkingProcess", "Thinking process")} text={(content as any).thinking} showIndicator={isLastContent && showIndicator} />;
          }
          if (content.type === "text") {
            return <AssistantBlockCard key={idx} title={t("chat.assistantLabel", "Assistant")} text={content.text} showIndicator={isLastContent && showIndicator} />;
          }
          if (content.type === "tool_use") {
            if (content.name === "AskUserQuestion") {
              return <AskUserQuestionCard key={idx} messageContent={content} permissionRequest={permissionRequest} onPermissionResult={onPermissionResult} provider={provider} />;
            }
